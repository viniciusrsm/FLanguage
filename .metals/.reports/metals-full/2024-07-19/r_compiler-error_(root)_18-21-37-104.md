...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Ident(_CURSOR_s)
       tree position: line 19 of file:///C:/Users/Vinicius/UnB/FLanguage/src/main/scala/br/unb/cic/flang/MonadState.scala
            tree tpe: <error>
...
== Source file context for tree position ==
    ...
    16 
    17     
    18     //def runState[A](state: M[A], initialState: S): (S, A) = state.run(initialState).value
    19     def runState[A](s: M[A]): (S => (S, A)) = s.run(_CURSOR_s).value
    20 
    21     def put(s: S): M[Unit] = State.set(s)
    22 
...
package object MonadState {
    ...
    //def runState[A](state: M[A], initialState: S): (S, A) = state.run(initialState).value
    def runState[A](s: M[A]): (S => (S, A)) = s.run(@@s).value
